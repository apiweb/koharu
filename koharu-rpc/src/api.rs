...
use koharu_core::{
    ...
    ExportResult, FontFaceInfo, ImportPathsRequest, JobState, LlmCatalog, LlmLoadRequest, LlmState,
...
async fn import_from_paths(
    ...
    let resources = state.resources()?;
    let replace = matches!(
        query.mode.unwrap_or(koharu_core::ImportMode::Append),
        koharu_core::ImportMode::Replace
    );

    io::import_from_paths(resources.clone(), body.paths, replace, body.insert_at)
        .await
        .map_err(|e| ApiError::bad_request(e.to_string()))?;
